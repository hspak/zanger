...
pub fn resize(
    ...
    // The repositioning and erase below are destructive, and resize events
    // frequently arrive in bursts during interactive drags. Presenting each
    // erased intermediate state made every burst flicker. Wrap them in
    // synchronized output so supporting terminals hold the frame until the
    // matching reset, and queue a full refresh so the following render()
    // repaints under synchronization as well.
