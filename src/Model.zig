...
const FileMetadata = struct {
    ...
    fn init(path: []const u8) !FileMetadata {
...
// widgets so it scrolls and clips consistently with directory contents.
const Preview = struct {
    ...
    fn initMessage(alloc: Allocator, message: []const u8) Allocator.Error!Preview {
...
// Stable row widgets let vxfw hit testing identify clicks without exposing
// ListView's private scroll state.
const Row = struct {
    // Borrowed from the pane that owns this row's backing slice.
    pane: *Pane,
    index: usize,

    fn widget(self: *const Row) vxfw.Widget {
        return .{
            .userdata = @constCast(self),
            .eventHandler = Row.typeErasedEventHandler,
...
const Pane = struct {
    ...
    fn init(self: *Pane, model: *Model, role: PaneRole) void {
        self.* = .{
            .model = model,
            .role = role,
            .listing = null,
    ...
    fn widget(self: *Pane) vxfw.Widget {
        return .{
            .userdata = self,
            .captureHandler = Pane.typeErasedCaptureHandler,
            .drawFn = Pane.typeErasedDrawFn,
        };
    }
    ...
    fn resetListView(self: *Pane, cursor: u32) void {
    ...
    fn replace(self: *Pane, replacement: Replacement) void {
    ...
    fn itemCount(self: *const Pane) usize {
